...
function parse(string) {
  ...
  while (!stream.eof()) {
    ...
    if (code === ESCAPE) {
      ...
      (field = consumeField(stream, cleanString.length + pos2 - offset))
    ) {
      fields.push(field)
...
function consumePlaceholder(stream) {
  ...
  if (stack.length) {
    throw stream.error(
      'Unable to find matching "}" for curly brace at ' + stack.pop()
    )
  }
...
function run(nodes, formatter, fieldsRenderer) {
  return nodes
    .map(function (node) {
      ...
      return outNode
        ? outNode.toString(run(node.children, formatter, fieldsRenderer))
        : ''
    })
    .join('')
}
...
var prototypeAccessors$1 = {
  ...
  attributesMap: { configurable: true },
  isGroup: { configurable: true },
  isTextOnly: { configurable: true },
  firstChild: { configurable: true },
  lastChild: { configurable: true },
  childIndex: { configurable: true },
  nextSibling: { configurable: true },
  previousSibling: { configurable: true },
  classList: { configurable: true },
}
...
Node.prototype.replaceAttribute = function replaceAttribute(
  curAttribute,
  newName,
  newValue
) {
...
Color.prototype.toHex = function toHex$1(short) {
  var fn =
    short && isShortHex(this.r) && isShortHex(this.g) && isShortHex(this.b)
      ? toShortHex
      : toHex
...
Profile.prototype.isBooleanAttribute = function isBooleanAttribute(attr) {
  return (
    attr.options.boolean ||
    this.get('booleanAttributes').indexOf((attr.name || '').toLowerCase()) !==
      -1
  )
}
          ...
          if (column - 1 > tokens[i].offset) {
            if (isLegalToken(tokens, i)) {
              ...
              setArr = getLegalEmmetSets(
                model.getLineContent(lineNumber).substring(offset, column - 1)
              )
            }
            break
          }
...
var option = __assign(__assign({}, defaultOption), {
    ...
    while (!stream.eof()) {
      ...
      if (ch === TEXT_START) {
        ...
        if (ch === TEXT_START || ch === TEXT_END) {
          result +=
            stream.substring(offset, stream.pos - 2) + String.fromCharCode(ch)
          offset = stream.pos
        }
      }
    }
...
function isUnquoted(code) {
  return (
    !isSpace(code) && !isQuote(code) && code !== ATTR_CLOSE && code !== EQUALS
  )
}
var HASH$1 = 35
var DOT$1$1 = 46
...
function consumeElement(stream) {
  ...
  if (start === stream.pos) {
    throw stream.error(
      'Unable to consume abbreviation node, unexpected ' + stream.peek()
    )
  }
  return node
}
...
function consumeText$1(stream) {
  ...
  if (stream.eat(TEXT_START$1)) {
    ...
    throw stream.error(
      'Unable to find closing ' +
        String.fromCharCode(TEXT_END$1) +
        ' for text start'
    )
  }
  return null
...
function template(str2, data) {
  ...
  while (i < str2.length) {
    ...
    if (code === TOKEN_OPEN) {
      ...
      if (!stack.length) {
        output +=
          str2.slice(offset, lastPos) +
          str2.slice(lastPos + 1, i).replace(TOKEN, replacer)
        offset = i + 1
      }
    }
    i++
    ...
    if (!handlePseudoSnippet(outNode)) {
      ...
      if (node.name) {
        ...
        outNode.open =
          ...
          name +
          attrs +
          (node.selfClosing ? profile.selfClose() : '') +
          '>'
        if (!node.selfClosing) {
          outNode.close = '</' + name + '>'
        }
...
function formatAttributes(outNode, profile) {
  ...
  return node.attributes
    .map(function (attr) {
      ...
      if (
        attr.options.boolean ||
        profile.get('booleanAttributes').indexOf(attrName.toLowerCase()) !== -1
      ) {
      ...
      return attr.options.before && attr.options.after
        ? ' ' +
            attrName +
            '=' +
            (attr.options.before + attrValue + attr.options.after)
...
function indentFormat(outNode, profile, options) {
  ...
  if (node.name) {
    ...
    if (
      options.omitName &&
      options.omitName.test(data.NAME) &&
      data.PRIMARY_ATTRS
    ) {
      data.NAME = null
    }
  ...
  node.attributes.forEach(function (attr) {
    ...
    if (reId.test(name)) {
      ...
      var isBoolean =
        attr.value == null &&
        (attr.options.boolean ||
          profile.get('booleanAttributes').indexOf(name.toLowerCase()) !== -1)
  ...
  var nodeOptions = {
    ...
    attributes: {
      secondary: function secondary(attrs) {
        return attrs
          .map(function (attr) {
            return attr.isBoolean
              ? '' +
                  attr.name +
                  (profile.get('compactBooleanAttributes') ? '' : '=true')
...
function slim(tree, profile, options) {
  options = options || {}
  var SECONDARY_ATTRS =
    (options.attributeWrap && secondaryAttrs[options.attributeWrap]) ||
    secondaryAttrs.none
  var booleanAttr =
    SECONDARY_ATTRS === secondaryAttrs.none
      ? function (attr) {
...
function updateFormatting$1(outNode, profile) {
  ...
  var parent = node.parent
  if (
    profile.get('inlineBreak') === 0 &&
    isInline$1(node, profile) &&
    !isRoot(parent) &&
    parent.value == null &&
...
function formatNodeValue$1(node, profile) {
  if (node.value != null && reNl$1.test(node.value)) {
    var indent = profile.indent(1)
    return splitByLines$1(node.value)
      .map(function (line, i) {
...
function resolveImplicitName(parentName) {
  ...
  return (
    elementMap[parentName] || (inlineElements.has(parentName) ? 'span' : 'div')
  )
}
...
function replaceRanges(str2, ranges, value) {
  for (var i = ranges.length - 1; i >= 0; i--) {
    ...
    if (str2.substr(r2[0] + r2[1], 1) === '@') {
      ...
      var matches = str2
        .substr(r2[0] + r2[1] + 1 + Number(descendingOrder))
        .match(/^(\d+)/)
      if (matches) {
    ...
    str2 =
      str2.substring(0, r2[0]) +
      (typeof value === 'function'
        ? value(str2.substr(r2[0], r2[1]), offset, descendingOrder)
        : value) +
      str2.substring(r2[0] + r2[1] + offsetLength)
  }
  return str2
}
...
function getNumberingRanges(str2) {
  return findUnescapedTokens(str2 || '', numberingToken).reduce(function (
    out2,
    range$$1
  ) {
...
function replaceNumberingRanges(str2, ranges, value, count) {
  var replaced = replaceRanges(
    str2,
    ranges,
    function (token, offset, descendingOrder) {
      var _value = descendingOrder
        ? String(offset + count - value + 1)
        : String(value + offset)
...
function insertContentIntoPlaceholder(node, content) {
  ...
  node.attributes.forEach(function (attr) {
    if (attr.value) {
      node.setAttribute(
        attr.name,
        replacePlaceholder(attr.value, content, state)
      )
    }
  })
...
function setNodeContent(node, content) {
  ...
  if (node.name.toLowerCase() === 'a' || node.hasAttribute('href')) {
    if (reUrl.test(content)) {
      node.setAttribute(
        'href',
        (reProto.test(content) ? '' : 'http://') + content
      )
...
var xsl = function (tree) {
  tree.walk(function (node) {
    if (
      reSupporterNames.test(node.name || '') &&
...
var htmlSnippet = {
  ...
  'link:favicon':
    "link[rel='shortcut icon' type=image/x-icon href='${1:leadler-logo.ico}']",
...
registry.get(0).set(reLorem, function (node) {
  var option2 = {}
  var _a = node.name.match(reLorem),
    lang = _a[1],
    wordCount = _a[2]
  if (lang) {
    option2.lang = lang
